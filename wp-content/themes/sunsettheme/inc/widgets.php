...
{
  //Establecer el nombre del widget, descripción, etc...
  ...
  public function form( $instances ){
      echo '<p><strong>No options for this Widget!</strong><br/>You can control the fields of this Widget from <a href="./admin.php?page=juan_sunset">This Page</a></p>';
  }
...
/*
  ...
  Editar estilos de widgets de wordpress
 ...
 */
 ...
 /*
   ...
   Guardar las vistas de los posts

  */
 function sunset_save_post_views( $postID ) {

   $metaKey = 'sunset_post_view';
...
class Sunset_Popular_Posts_Widget extends WP_Widget {
  ...
  public function form( $instance ){
    ...
    $output .= '<input type="text" class="widefat" id="'. esc_attr( $this->get_field_id( 'title' ) ) .'" name="'. esc_attr( $this->
    get_field_name( 'title' ) ) .'" value="'.esc_attr($title).'"';
    $output .= '</p>';


    $output .= '<p>';
    $output .= '<label for="'. esc_attr( $this->get_field_id( 'tot' ) ) .'">Number of Posts:</label>';
    $output .= '<input type="number" class="widefat" id="'. esc_attr( $this->get_field_id( 'tot' ) ) .'" min="1" name="'. esc_attr( $this->
    get_field_name( 'tot' ) ) .'" value="'.esc_attr($tot).'"';
    $output .= '</p>';
      ...
      if ($posts_query->have_posts() ):
        ...
        //echo '<ul>';
            ...
            $format =  get_post_format() ? get_post_format() : 'standard';
            					...
            					echo '<div class="media-left"><img class="media-object" src="' .
            						get_template_directory_uri() . '/img/post-' . $format  .  '.png" alt="' .
            						get_the_title() . '" /></div>';
            					echo '<div class="media-body"><a href=' . get_permalink() .'>' . get_the_title() . '</a></div>';
            					echo '<div class="text-uppercase small">' . sunset_comment_footer() . '</div>';
            					echo '</div>';
